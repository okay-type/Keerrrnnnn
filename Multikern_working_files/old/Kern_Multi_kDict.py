...
probably dont need the add/remove added buttons thing, can just update


TODO Performance

Things to make into custom coalescable subscriber events:
...
kerning adjustments only update current font display
    on quick update, only update changed font, not all
...
class MultiKern(Subscriber, WindowController):
    ...
    def build(self):
        ...
        self.w.controls = Group((20, -88, 300, -0))
        ...
        self.w.controls.load = Button(
            (0, 32, 44, 22), 'Load', sizeStyle='mini', callback=self.loadPairlist)
        self.w.controls.save = Button(
            (44, 32, 44, 22), 'Save', sizeStyle='mini', callback=self.saveFiles)
        ...
        # self.w.controls.L.edit.setNextKeyView_(self.w.kerningMerzView.)
        # self.w.controls.R.edit.setNextKeyView_(self.w.kerningMerzView.nsObject)
        ...
        if self.hold is False:
            ...
            for notifyAt in [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1]:
                if pairindex/self.paircount < notifyAt and progress >= notifyAt:
                    ...
                    if notifyAt < 1:
                        print('The next benchmark pair is:', self.pairlist[int(self.paircount * (notifyAt+.1))])
