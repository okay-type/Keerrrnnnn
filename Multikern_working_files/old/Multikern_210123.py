...
FEATURE THAT SHOULD BE IN IT TO SHARE
use MM pairlist as backup
...
class Multikern(Subscriber, WindowController):
    ...
    def build_window_controls_universal(self, screenW, screenH):  # universal controls - right side
        ...
        pair.L_line = HorizontalLine((gutter, line-2, self.c(column_8th, gutter, 4)-(gutter*3), 1))
        pair.R_line = HorizontalLine((self.c(column_8th, gutter, 4)+gutter, line-2, -gutter, 1))
        ...
        universal.reference_z.line = HorizontalLine((gutter, 0, self.c(column_8th, gutter, 4)-(gutter*3), 1))
        universal.reference_z.zero = okButton(
            ...
            sizeStyle='mini')
        universal.reference_z.line2 = HorizontalLine((self.c(column_8th, gutter, 4)+gutter, 0, -gutter, 1))
    ...
    def fontlist_edit(self, sender):
        ...
        # window UI controls
        updatedufo_dict = []
        if len(fontlist) == len(self.ufolist):  # only get checkmark edits
            if fontlist != self.ufolist:  # only get checkmark edits
                for ufoitem in fontlist:
                    updatedufo = {}
        ...
        if len(fontlist) == len(self.ufolist):  # only get checkmark edits
            if fontlist != self.ufolist:  # only get checkmark edits
                for i, ufoitem in enumerate(fontlist):
                    ufo = ufoitem['ufo']
                    if ufoitem['spacecenter'] != self.ufolist[i-1]['spacecenter']:
                        if ufoitem['spacecenter'] is True:
                            self.fontlist_open_ufo_spacecenter(ufo, i)
                        if ufoitem['spacecenter'] is False:
                            self.fontlist_close_ufo_spacecenter(i)
                    if ufoitem['overview'] != self.ufolist[i-1]['overview']:
                        if ufoitem['overview'] is True:
                            self.fontlist_open_ufo_ui(ufo, i)
                        if ufoitem['overview'] is False:
                            self.fontlist_close_ufo_ui(ufo, i)
        print(fontlist)
        print(updatedufo_dict)
        ...
        if self.hold is False:
            self.ufolist = self.w.lists.fontlist.get()
            if self.listcount != len(self.ufolist):
                self.listcount = len(self.ufolist)
                for noUIfont in self.fonts_no_ui:
                    toclose = True
                    for font in self.ufolist:
                        if font['ufo'] == noUIfont:
                            toclose = False
                    if toclose is True:
                        print('closing', noUIfont.info.familyName + ' ' + noUIfont.info.styleName)
                        noUIfont.close()
                        self.fonts_no_ui.remove(noUIfont)
            postEvent(f'{MultikernKey}.merzLineView_hold', holdfor='change_fonts')
            postEvent(f'{MultikernKey}.merzLineView_change_fonts')
    ...
    # font window stuff like open spacecenter, open info, open overview


    def fontlist_open_ufo_spacecenter(self, ufo, i):
        sc = OpenSpaceCenter(ufo, newWindow=False)
        sc.set(self.string_builder())
        self.opened_windows[sc] = i

    def fontlist_close_ufo_spacecenter(self, i):
    ...
    def fontlist_open_ufo_ui(self, ufo, i):
        self.fonts_ui.append(ufo)
        if ufo in self.fonts_no_ui:
            self.fonts_no_ui.remove(ufo)
        ufo.openInterface()
        w = ufo.document().getMainWindow()
        w.bind('should close', self.ufo_ui_window_should_close)
        self.opened_windows[w] = i

    def fontlist_close_ufo_ui(self, ufo, i):
    ...
    def ufo_ui_window_should_close(self, sender):
        print('window_should_close', sender)
        # uncheck overview checkbox
        w = sender
        if w in self.opened_windows.keys():
            i = self.opened_windows[w]
            del self.opened_windows[w]
            ufolist_updated = self.w.lists.fontlist.get()
            ufolist_updated[i]['overview'] = False
            self.w.lists.fontlist.set(ufolist_updated)
        # soft close
        w = ufo.document().getMainWindow()
        w.close()
    ...
    def pairlist_selection_changed(self, sender):
        if self.hold is False:
            ...
            for notifyAt in self.notify_steps:
    ...
    def report_progress(self, i):
        ...
        for i, notifyAt in enumerate(self.notify_steps):
            if progress > notifyAt and progress <= self.notify_steps[i]:
    ...
    def string_builder(self):
        ...
        if '.sc' in g:
            H = 'h.sc'
        elif self.is_this_lower(g) is True:
            H = 'n'
    ...
    def is_this_lower(self, g):
        answer = False
        if g.islower() and len(g) == 1:
            answer = True
        # what about extended lowercase? eth agrave etc?
        elif len(self.ufolist) > 0:
            # use first font
            if g in self.ufolist[0]['ufo'].glyphOrder:
                uni = self.ufolist[0]['ufo'][g].unicode
                if uni is not None:
        ...
        if i == 'AllFonts':
            for n in range(len(self.ufolist)):
                ...
                if n+1 == len(self.ufolist):
                    hold = False
                kern = self.ufolist[n]['ufo'].kerning.find(pair) or 0
