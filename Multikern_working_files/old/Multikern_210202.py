...
class Multikern(Subscriber, WindowController):
    ...
    def build_window_lists(self, screenW, screenH):  # lists - left side 
        ...
        self.w.lists.metric_nudge_nL = Button(
            (0, -line, column/6, line),
            '-R',
            sizeStyle='mini',
            callback=self.metric_nudge_nL,
        )
        self.w.lists.metric_nudge_pL = Button(
            (column/6, -line, column/6, line),
            '+R',
            sizeStyle='mini',
            callback=self.metric_nudge_pL,
        )
        self.w.lists.metric_nudge_glyph = okEditText(
            (column/3, -line+6, column/3, line),
            'Glyph',
            align='center', 
            sizeStyle='mini',
        )
        self.w.lists.metric_nudge_nR = Button(
            (column/3*2, -line, column/6, line),
            '-L',
            sizeStyle='mini',
            callback=self.metric_nudge_nR,
        )
        self.w.lists.metric_nudge_pR = Button(
            (column/6*5, -line, column/6, line),
            '+L',
            sizeStyle='mini',
    ...
    def pairlist_selection_changed(self, sender):
        if self.hold is False:
            ...
            self.w.lists.metric_nudge_glyph.set(self.pair_current[0])
    ...
    def string_builder(self):
        ...
        if '.numr' in self.pair_current[1]:
            text = [H, H, H, H, H, self.pair_current[0], self.pair_current[1], H, H, H, self.pair_flipped[0], self.pair_flipped[1], H, H, H, H, H]
    ...
    def metric_nudge(self, amount, side):
        g = self.w.lists.metric_nudge_glyph.get()
        selection = self.w.lists.fontlist.getSelection()
        for i, f in enumerate(self.ufolist):
            if i in selection:
                ufo = f['ufo']
