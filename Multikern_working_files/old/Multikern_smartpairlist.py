...
class Multikern(Subscriber, WindowController):
    ...
    def fontlist_edit(self, sender):  # self.w.lists.fontlist callback
        for box in sender.get():
            if box['visibile'] is True:
                if box['ufo'] in self.no_UI_ufos:
                    self.no_UI_ufos.remove(box['ufo'])
                    self.fontlist_open_ufo_ui(box['ufo'])
            if box['visibile'] is False:
                if box['ufo'] not in self.no_UI_ufos:
                    self.no_UI_ufos.append(box['ufo'])
                    self.fontlist_close_ufo_ui(box['ufo'])
