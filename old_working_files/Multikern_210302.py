...
class Multikern(Subscriber, WindowController):
    ...
    def whichfonts(self):  # reutnrs list of ufos
        # 0 'All',
        # 1 'Selected',
        # 2 'Unselected',
        # 3 'Checked',
    ...
    def string_builder(self):
        ...
        if '.sups' in self.pair_current[0] or 'superior' in self.pair_current[0]:
            if '.sups' in self.pair_current[1] or 'superior' in self.pair_current[1]:
                H = 'H'
                for g in ('zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine'):
                    if g in self.pair_current[0] or g in self.pair_current[1]:
                        B = 'zero.sups'
                        break
                    else:
                        B = 'n.sups'
        if '.subs' in self.pair_current[0] and '.subs' in self.pair_current[1]:
            H = 'H'
            for g in ('zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine'):
                if g in self.pair_current[0] or g in self.pair_current[1]:
                    B = 'zero.subs'
        ...
        # check if fractions
        H = 'H'
        if '.numr' in self.pair_current[0] and '.numr' in self.pair_current[1]:
            text = [H, H, H, 'zero.numr', 'zero.numr', self.pair_current[0], self.pair_current[1], H, H, self.pair_flipped[0], self.pair_flipped[1], 'zero.numr', 'zero.numr', H, H, H]
        if '.dnom' in self.pair_current[0] and '.dnom' in self.pair_current[1]:
            text = [H, H, H, 'zero.dnom', 'zero.dnom', self.pair_current[0], self.pair_current[1], H, H, self.pair_flipped[0], self.pair_flipped[1], 'zero.dnom', 'zero.dnom', H, H, H]
        if '.numr' in self.pair_current[1] and '.numr' not in self.pair_current[0]:
            text = [H, H, H, H, H, self.pair_current[0], self.pair_current[1], 'fraction', self.pair_flipped[0], self.pair_flipped[1], H, H, H, H, H]
        if '.numr' in self.pair_current[0] and 'fraction' in self.pair_current[1]:
            text = [H, H, H, H, 'zero.numr', 'zero.numr', self.pair_current[0], self.pair_current[1], self.pair_flipped[1], 'zero.dnom', 'zero.dnom',H, H, H, H]
