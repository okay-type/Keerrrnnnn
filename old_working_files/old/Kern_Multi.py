...
class MultiKern(Subscriber):
    ...
    def pairlistSelectionCallback(self, sender):
        if self.hold is False:
            ...
            for notifyAt in [.1, .2, .3, .4, .5, .6, .7, .8, .9, 1]:
                if pairindex/self.paircount < notifyAt and progress >= notifyAt:
                    PostBannerNotification('Robofont', 'You are' + str(progress) + '% done with this kerning list!')
    ...
    def buildKernUI(self):
        ...
        self.kernUIData['smallLayerGlyphnames'] = []
        self.kernUIData['smallGlyphLayers'] = []
        ...
        with baseLayer.sublayerGroup():
            ...
            smallLayer.addSublayerScaleTransformation(
                self.smallFontSize / 1000, name='scale', center=(0, 0))
            ...
            for i, font in enumerate(self.fontlist):
                ...
                fontlayer.addSublayerScaleTransformation(
                    self.bigFontSize / 1000, name='scale', center=(0, 0))
    ...
    def makeKernUIPositionData(self):
        ...
        for i, fontData in enumerate(self.kernUIData['fontData']):
            ...
            textSmall = [H, H, self.pairCurrent[0], self.pairCurrent[1], B, self.pairFlipped[0], self.pairFlipped[1], H]
            ...
            kernvalue = 0
            for n, glyphName in enumerate(textSmall):
                if glyphName in ufoGlyphOrder:
                    self.kernUIData['smallLayerGlyphnames'].append([glyphName, i])
                    self.kernUIData['smallLayerXadvance'].append(smallx)
                    smallx += ufo[glyphName].width
                    kernvalue = 0
                    if n < len(textSmall) - 1:
                        kernvalue = ufo.kerning.find((glyphName, textSmall[n + 1])) or 0
                        smallx += kernvalue
    ...
    def addKernUIGlyphLayers(self):
        ...
        with baseLayer.sublayerGroup():
            ...
            for x in self.kernUIData['smallLayerGlyphnames']:
                glyphName, i = x
                ufo = self.kernUIData['fontData'][i]['ufo']
                smallGlyphLayer = smallLayer.appendPathSublayer(position=(0, 0),)
                glyphPath = ufo[glyphName].getRepresentation('merz.CGPath')
                smallGlyphLayer.setPath(glyphPath)
                self.kernUIData['smallGlyphLayers'].append(smallGlyphLayer)
    ...
    def positionKernUIGlyphLayers(self):
        ...
        for i, smallGlyphLayer in enumerate(self.kernUIData['smallGlyphLayers']):
            ...
            if kern != 0:
                ...
                smallGlyphLayer.appendTextLineSublayer(
                    position=(
                        self.kernUIData['smallLayerXadvance'][i + 1] - x, self.labelOffsetBig),
                    size=(10, 1),
                    pointSize=8,
                    text=str(kern),
                    fillColor=(*c,),
